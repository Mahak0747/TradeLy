"use strict";
/* tslint:disable */
/* eslint-disable */
/**
 * Twelve Data API client for Node.js
 *
 * NOTE: This code is auto generated, please do not edit it manually.
 */
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.twelvedataErrorMiddleware = exports.InternalServerError = exports.TooManyRequestsError = exports.ParameterTooLongError = exports.NotFoundError = exports.ForbiddenError = exports.UnauthorizedError = exports.BadRequestError = exports.TwelvedataApiError = void 0;
/**
 * Base exception for all Twelvedata API errors.
 */
class TwelvedataApiError extends Error {
    constructor(statusCode, code, status, message) {
        super(message);
        this.statusCode = statusCode;
        this.code = code;
        this.status = status;
        this.name = "TwelvedataApiError";
    }
}
exports.TwelvedataApiError = TwelvedataApiError;
class BadRequestError extends TwelvedataApiError {
    constructor(code, status, message) {
        super(400, code, status, message);
        this.name = "BadRequestError";
    }
}
exports.BadRequestError = BadRequestError;
class UnauthorizedError extends TwelvedataApiError {
    constructor(code, status, message) {
        super(401, code, status, message);
        this.name = "UnauthorizedError";
    }
}
exports.UnauthorizedError = UnauthorizedError;
class ForbiddenError extends TwelvedataApiError {
    constructor(code, status, message) {
        super(403, code, status, message);
        this.name = "ForbiddenError";
    }
}
exports.ForbiddenError = ForbiddenError;
class NotFoundError extends TwelvedataApiError {
    constructor(code, status, message) {
        super(404, code, status, message);
        this.name = "NotFoundError";
    }
}
exports.NotFoundError = NotFoundError;
class ParameterTooLongError extends TwelvedataApiError {
    constructor(code, status, message) {
        super(414, code, status, message);
        this.name = "ParameterTooLongError";
    }
}
exports.ParameterTooLongError = ParameterTooLongError;
class TooManyRequestsError extends TwelvedataApiError {
    constructor(code, status, message) {
        super(429, code, status, message);
        this.name = "TooManyRequestsError";
    }
}
exports.TooManyRequestsError = TooManyRequestsError;
class InternalServerError extends TwelvedataApiError {
    constructor(code, status, message) {
        super(500, code, status, message);
        this.name = "InternalServerError";
    }
}
exports.InternalServerError = InternalServerError;
const ERROR_MAP = {
    400: (b) => new BadRequestError(b.code, b.status, b.message),
    401: (b) => new UnauthorizedError(b.code, b.status, b.message),
    403: (b) => new ForbiddenError(b.code, b.status, b.message),
    404: (b) => new NotFoundError(b.code, b.status, b.message),
    414: (b) => new ParameterTooLongError(b.code, b.status, b.message),
    429: (b) => new TooManyRequestsError(b.code, b.status, b.message),
    500: (b) => new InternalServerError(b.code, b.status, b.message),
};
function isErrorResponseBody(data) {
    return (typeof data === "object" &&
        data !== null &&
        typeof data.code === "number" &&
        typeof data.message === "string" &&
        typeof data.status === "string");
}
/**
 * Middleware that translates non-2xx Twelvedata API responses into
 * typed `TwelvedataApiError` subclasses. Attached by default to
 * `Configuration` instances created via `CreateConfig`.
 *
 * Uses `response.clone()` because `Response.body` is a single-use stream
 * and downstream consumers may still need to read it.
 */
exports.twelvedataErrorMiddleware = {
    post: (context) => __awaiter(void 0, void 0, void 0, function* () {
        const { response } = context;
        if (response.ok)
            return;
        let body;
        try {
            body = yield response.clone().json();
        }
        catch (_a) {
            return;
        }
        if (!isErrorResponseBody(body))
            return;
        const resolver = ERROR_MAP[response.status];
        if (resolver) {
            throw resolver(body);
        }
        throw new TwelvedataApiError(response.status, body.code, body.status, body.message);
    }),
};
