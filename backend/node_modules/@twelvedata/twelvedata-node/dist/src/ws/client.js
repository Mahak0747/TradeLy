"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TwelvedataWebSocketClient = void 0;
/**
 * Twelvedata WebSocket client.
 *
 * Wraps the `ws` library to provide:
 *   - real-time price streaming via Node's `EventEmitter` interface
 *   - application-level heartbeat (`{"action":"heartbeat"}`)
 *   - WebSocket-protocol ping/pong with timeout-based dead-connection detection
 *   - automatic reconnection with exponential backoff
 *   - automatic re-subscription to active symbols after reconnect
 */
const events_1 = require("events");
const ws_1 = __importDefault(require("ws"));
const constants_1 = require("./constants");
const errors_1 = require("./errors");
class TwelvedataWebSocketClient extends events_1.EventEmitter {
    constructor(options = {}) {
        var _a, _b, _c, _d, _e;
        super();
        this.ws = null;
        this.heartbeatTimer = null;
        this.pingTimer = null;
        this.pongTimeoutTimer = null;
        this.reconnectTimer = null;
        this.reconnectAttempts = 0;
        this.userInitiatedClose = false;
        /**
         * True once the socket has reached OPEN at least once. Gates the
         * auto-reconnect loop so a failed initial `connect()` does not silently
         * spawn background reconnect attempts the caller never asked for.
         */
        this.hasEverConnected = false;
        /**
         * Tracks an in-flight `openSocket()` promise so concurrent `connect()`
         * calls fold into a single attempt instead of opening multiple sockets.
         */
        this.connectingPromise = null;
        /**
         * True if the most recent `openSocket()` call actually constructed a
         * `WebSocket`. Lets the reconnect loop tell apart synchronous failures
         * (no `close` event will fire — we must reschedule ourselves) from
         * async failures (the socket's own `close` handler reschedules).
         */
        this.lastAttemptCreatedSocket = false;
        /**
         * Set when an error occurs that retrying cannot fix (e.g. invalid API key).
         * Suppresses the auto-reconnect loop and is exposed via `connect()`'s rejection.
         */
        this.fatalError = null;
        /**
         * Keyed by canonical JSON form of the subscription so that string and
         * extended-object inputs that resolve to the same instrument are deduped.
         */
        this.subscriptions = new Map();
        const apiKey = (_a = options.apiKey) !== null && _a !== void 0 ? _a : process.env.TWELVEDATA_API_KEY;
        if (!apiKey) {
            throw new errors_1.WebSocketAuthError("TWELVEDATA_API_KEY environment variable is not set and no apiKey option was provided");
        }
        this.apiKey = apiKey;
        this.url = (_b = options.url) !== null && _b !== void 0 ? _b : constants_1.DEFAULT_WS_URL;
        this.heartbeatIntervalMs =
            (_c = options.heartbeatIntervalMs) !== null && _c !== void 0 ? _c : constants_1.DEFAULT_HEARTBEAT_INTERVAL_MS;
        this.pingIntervalMs = (_d = options.pingIntervalMs) !== null && _d !== void 0 ? _d : constants_1.DEFAULT_PING_INTERVAL_MS;
        this.pingTimeoutMs = (_e = options.pingTimeoutMs) !== null && _e !== void 0 ? _e : constants_1.DEFAULT_PING_TIMEOUT_MS;
        this.reconnectConfig = resolveReconnect(options.reconnect);
    }
    /**
     * Open the WebSocket connection. Resolves on `open`, rejects if the very
     * first attempt fails. Background reconnects after a successful first open
     * are non-throwing and surface via the `reconnecting` / `reconnect` / `error` events.
     */
    connect() {
        if (this.ws && this.ws.readyState === ws_1.default.OPEN) {
            return Promise.resolve();
        }
        if (this.connectingPromise) {
            return this.connectingPromise;
        }
        this.userInitiatedClose = false;
        this.fatalError = null;
        const attempt = this.openSocket().finally(() => {
            this.connectingPromise = null;
        });
        this.connectingPromise = attempt;
        return attempt;
    }
    /** True if the underlying socket is in the OPEN state. */
    isConnected() {
        return this.ws !== null && this.ws.readyState === ws_1.default.OPEN;
    }
    /**
     * Subscribe to one or more symbols. Tracked internally so the subscription
     * is automatically replayed after a reconnect.
     */
    subscribe(input) {
        const items = normalizeInput(input);
        if (items.length === 0)
            return;
        for (const item of items) {
            this.subscriptions.set(canonicalKey(item), item);
        }
        this.send({
            action: "subscribe",
            params: { symbols: serializeSymbols(items) },
        });
    }
    /** Unsubscribe from one or more symbols and stop tracking them. */
    unsubscribe(input) {
        const items = normalizeInput(input);
        if (items.length === 0)
            return;
        for (const item of items) {
            this.subscriptions.delete(canonicalKey(item));
        }
        this.send({
            action: "unsubscribe",
            params: { symbols: serializeSymbols(items) },
        });
    }
    /** Reset the server-side subscription list and clear local tracking. */
    reset() {
        this.subscriptions.clear();
        this.send({ action: "reset" });
    }
    /** Send a single application-level heartbeat frame. */
    heartbeat() {
        this.send({ action: "heartbeat" });
    }
    /**
     * Close the connection cleanly and disable automatic reconnect. After this
     * the client is inert until `connect()` is called again. Tracked
     * subscriptions are preserved and will be replayed on the next successful
     * `connect()`; call `reset()` first if you want to discard them.
     */
    disconnect() {
        this.userInitiatedClose = true;
        this.clearReconnectTimer();
        this.stopHeartbeat();
        this.stopPing();
        this.reconnectAttempts = 0;
        this.fatalError = null;
        if (this.ws) {
            try {
                this.ws.close(1000, "client disconnect");
            }
            catch (_a) {
                // ignore — terminate as a fallback
                try {
                    this.ws.terminate();
                }
                catch (_b) {
                    /* noop */
                }
            }
        }
    }
    // ---- internal ----------------------------------------------------------
    openSocket() {
        this.lastAttemptCreatedSocket = false;
        return new Promise((resolve, reject) => {
            let settled = false;
            // Set synchronously by `unexpected-response` so a racing `error`
            // event doesn't pre-empt the auth/non-auth classification.
            let classifyingUpgrade = false;
            let socket;
            try {
                socket = new ws_1.default(this.buildUrl());
            }
            catch (err) {
                reject(new errors_1.WebSocketConnectionError("Failed to open WebSocket", err));
                return;
            }
            this.ws = socket;
            this.lastAttemptCreatedSocket = true;
            const onOpen = () => {
                this.reconnectAttempts = 0;
                this.hasEverConnected = true;
                this.startHeartbeat();
                this.startPing();
                this.replaySubscriptions();
                this.emit("open");
                if (!settled) {
                    settled = true;
                    resolve();
                }
            };
            const onMessage = (data) => {
                this.handleMessage(data);
            };
            const onError = (err) => {
                // `error` is always followed by `close`; defer reconnect handling there.
                // If `unexpected-response` already classified the failure as fatal,
                // skip the duplicate generic error.
                if (this.fatalError) {
                    if (!settled) {
                        settled = true;
                        reject(this.fatalError);
                    }
                    return;
                }
                // An upgrade-failure classification is in flight — let it finish
                // so an auth failure isn't misreported as a generic connection error.
                if (classifyingUpgrade) {
                    return;
                }
                this.emit("error", err);
                if (!settled) {
                    settled = true;
                    reject(new errors_1.WebSocketConnectionError(err.message, err));
                }
            };
            const onClose = (code, reasonBuf) => {
                var _a;
                const reason = (_a = reasonBuf === null || reasonBuf === void 0 ? void 0 : reasonBuf.toString()) !== null && _a !== void 0 ? _a : "";
                this.stopHeartbeat();
                this.stopPing();
                this.emit("close", code, reason);
                this.ws = null;
                if (!this.userInitiatedClose && this.hasEverConnected) {
                    this.scheduleReconnect();
                }
            };
            const onPong = () => {
                this.clearPongTimeout();
            };
            const onUnexpectedResponse = (_req, res) => {
                var _a;
                classifyingUpgrade = true;
                const chunks = [];
                let finalized = false;
                const finalize = (err) => {
                    if (finalized)
                        return;
                    finalized = true;
                    this.fatalError = err;
                    this.emit("error", err);
                    if (!settled) {
                        settled = true;
                        reject(err);
                    }
                    // Force the socket closed so the close handler fires (which will
                    // skip reconnect because `fatalError` is set).
                    try {
                        socket.terminate();
                    }
                    catch (_a) {
                        /* noop */
                    }
                };
                // Safety net: if the response stream stalls (no `end`), classify
                // from the status line alone after a short grace period.
                const safetyTimer = setTimeout(() => {
                    var _a;
                    const status = (_a = res.statusCode) !== null && _a !== void 0 ? _a : 0;
                    finalize(classifyUpgradeFailure(status, ""));
                }, 2000);
                (_a = safetyTimer.unref) === null || _a === void 0 ? void 0 : _a.call(safetyTimer);
                res.on("data", (chunk) => chunks.push(chunk));
                res.on("end", () => {
                    var _a;
                    clearTimeout(safetyTimer);
                    const status = (_a = res.statusCode) !== null && _a !== void 0 ? _a : 0;
                    const body = Buffer.concat(chunks).toString("utf8");
                    finalize(classifyUpgradeFailure(status, body));
                });
            };
            socket.on("open", onOpen);
            socket.on("message", onMessage);
            socket.on("error", onError);
            socket.on("close", onClose);
            socket.on("pong", onPong);
            socket.on("unexpected-response", onUnexpectedResponse);
        });
    }
    buildUrl() {
        const u = new URL(this.url);
        u.searchParams.set("apikey", this.apiKey);
        return u.toString();
    }
    handleMessage(data) {
        let parsed;
        try {
            const text = typeof data === "string" ? data : data.toString();
            parsed = JSON.parse(text);
        }
        catch (err) {
            this.emit("error", new errors_1.TwelvedataWebSocketError(`Failed to parse WebSocket message: ${err.message}`));
            return;
        }
        switch (parsed.event) {
            case "price":
                this.emit("price", parsed);
                break;
            case "subscribe-status": {
                const event = parsed;
                // Server may return `null` instead of `[]` — normalize so callers
                // can rely on the documented array type.
                event.success = Array.isArray(event.success) ? event.success : [];
                event.fails = Array.isArray(event.fails) ? event.fails : [];
                this.emit("subscribe-status", event);
                break;
            }
            case "heartbeat":
                this.emit("heartbeat", parsed);
                break;
            default:
                this.emit("message", parsed);
        }
    }
    send(payload) {
        if (!this.ws || this.ws.readyState !== ws_1.default.OPEN) {
            this.emit("error", new errors_1.WebSocketConnectionError("Cannot send: WebSocket is not open"));
            return;
        }
        try {
            this.ws.send(JSON.stringify(payload));
        }
        catch (err) {
            this.emit("error", new errors_1.WebSocketConnectionError(`Failed to send WebSocket frame: ${err.message}`, err));
        }
    }
    startHeartbeat() {
        var _a, _b;
        this.stopHeartbeat();
        if (this.heartbeatIntervalMs <= 0)
            return;
        this.heartbeatTimer = setInterval(() => {
            this.heartbeat();
        }, this.heartbeatIntervalMs);
        // Don't keep the Node event loop alive solely for the heartbeat.
        (_b = (_a = this.heartbeatTimer).unref) === null || _b === void 0 ? void 0 : _b.call(_a);
    }
    stopHeartbeat() {
        if (this.heartbeatTimer) {
            clearInterval(this.heartbeatTimer);
            this.heartbeatTimer = null;
        }
    }
    startPing() {
        var _a, _b;
        this.stopPing();
        if (this.pingIntervalMs <= 0)
            return;
        this.pingTimer = setInterval(() => {
            if (!this.ws || this.ws.readyState !== ws_1.default.OPEN)
                return;
            try {
                this.ws.ping();
            }
            catch (_a) {
                // If the ping itself throws, force a reconnect path.
                this.handleDeadConnection();
                return;
            }
            this.armPongTimeout();
        }, this.pingIntervalMs);
        (_b = (_a = this.pingTimer).unref) === null || _b === void 0 ? void 0 : _b.call(_a);
    }
    stopPing() {
        if (this.pingTimer) {
            clearInterval(this.pingTimer);
            this.pingTimer = null;
        }
        this.clearPongTimeout();
    }
    armPongTimeout() {
        var _a, _b;
        this.clearPongTimeout();
        if (this.pingTimeoutMs <= 0)
            return;
        this.pongTimeoutTimer = setTimeout(() => {
            this.emit("error", new errors_1.WebSocketTimeoutError(`No pong received within ${this.pingTimeoutMs}ms`, this.pingTimeoutMs));
            this.handleDeadConnection();
        }, this.pingTimeoutMs);
        (_b = (_a = this.pongTimeoutTimer).unref) === null || _b === void 0 ? void 0 : _b.call(_a);
    }
    clearPongTimeout() {
        if (this.pongTimeoutTimer) {
            clearTimeout(this.pongTimeoutTimer);
            this.pongTimeoutTimer = null;
        }
    }
    handleDeadConnection() {
        if (!this.ws)
            return;
        try {
            this.ws.terminate();
        }
        catch (_a) {
            /* noop — close handler will still run */
        }
        // The `close` event will trigger scheduleReconnect.
    }
    scheduleReconnect() {
        var _a, _b;
        if (!this.reconnectConfig.enabled)
            return;
        // Auth failures and other fatal errors cannot be fixed by retrying.
        if (this.fatalError)
            return;
        if (this.reconnectAttempts >= this.reconnectConfig.maxAttempts) {
            this.emit("error", new errors_1.WebSocketReconnectError(`Reconnect failed after ${this.reconnectAttempts} attempts`, this.reconnectAttempts));
            return;
        }
        const attempt = this.reconnectAttempts + 1;
        const baseDelay = Math.min(this.reconnectConfig.initialDelayMs *
            Math.pow(this.reconnectConfig.backoffFactor, this.reconnectAttempts), this.reconnectConfig.maxDelayMs);
        // Apply ±20% jitter so a fleet of clients reconnecting after the same
        // outage doesn't thundering-herd the server on a synchronized cadence.
        const jitter = baseDelay * 0.2 * (Math.random() * 2 - 1);
        const delay = Math.max(0, Math.round(baseDelay + jitter));
        this.reconnectAttempts = attempt;
        this.emit("reconnecting", { attempt, delayMs: delay });
        this.clearReconnectTimer();
        this.reconnectTimer = setTimeout(() => {
            this.reconnectTimer = null;
            this.openSocket()
                .then(() => {
                // `onOpen` already replayed tracked subscriptions.
                this.emit("reconnect", { attempt });
            })
                .catch(() => {
                // openSocket already emitted `error`. If a socket actually
                // reached the event loop, its `close` handler will re-trigger
                // the reconnect loop. If `new WebSocket` failed synchronously
                // there is no close event coming — schedule the next attempt
                // ourselves so the loop keeps making progress.
                if (!this.lastAttemptCreatedSocket &&
                    !this.userInitiatedClose &&
                    !this.fatalError) {
                    this.scheduleReconnect();
                }
            });
        }, delay);
        (_b = (_a = this.reconnectTimer).unref) === null || _b === void 0 ? void 0 : _b.call(_a);
    }
    clearReconnectTimer() {
        if (this.reconnectTimer) {
            clearTimeout(this.reconnectTimer);
            this.reconnectTimer = null;
        }
    }
    replaySubscriptions() {
        if (this.subscriptions.size === 0)
            return;
        const items = Array.from(this.subscriptions.values());
        this.send({
            action: "subscribe",
            params: { symbols: serializeSymbols(items) },
        });
    }
}
exports.TwelvedataWebSocketClient = TwelvedataWebSocketClient;
// ---- helpers -------------------------------------------------------------
function resolveReconnect(opt) {
    var _a, _b, _c, _d;
    if (opt === false) {
        return {
            enabled: false,
            initialDelayMs: constants_1.DEFAULT_RECONNECT_INITIAL_DELAY_MS,
            maxDelayMs: constants_1.DEFAULT_RECONNECT_MAX_DELAY_MS,
            maxAttempts: 0,
            backoffFactor: constants_1.DEFAULT_RECONNECT_BACKOFF_FACTOR,
        };
    }
    return {
        enabled: true,
        initialDelayMs: (_a = opt === null || opt === void 0 ? void 0 : opt.initialDelayMs) !== null && _a !== void 0 ? _a : constants_1.DEFAULT_RECONNECT_INITIAL_DELAY_MS,
        maxDelayMs: (_b = opt === null || opt === void 0 ? void 0 : opt.maxDelayMs) !== null && _b !== void 0 ? _b : constants_1.DEFAULT_RECONNECT_MAX_DELAY_MS,
        maxAttempts: (_c = opt === null || opt === void 0 ? void 0 : opt.maxAttempts) !== null && _c !== void 0 ? _c : constants_1.DEFAULT_RECONNECT_MAX_ATTEMPTS,
        backoffFactor: (_d = opt === null || opt === void 0 ? void 0 : opt.backoffFactor) !== null && _d !== void 0 ? _d : constants_1.DEFAULT_RECONNECT_BACKOFF_FACTOR,
    };
}
function normalizeInput(input) {
    const raw = Array.isArray(input) ? input.filter(Boolean) : [input];
    const out = [];
    for (const item of raw) {
        if (typeof item === "string") {
            // Split comma-separated tickers so canonical-key dedup works at the
            // individual symbol level (e.g. "AAPL,MSFT" + "AAPL" → {AAPL, MSFT}).
            for (const part of item.split(",")) {
                const trimmed = part.trim();
                if (trimmed)
                    out.push(trimmed);
            }
        }
        else {
            out.push(item);
        }
    }
    return out;
}
/**
 * Build the value sent as `params.symbols`. A single string is sent as-is
 * (allowing callers to use the comma-separated form documented by Twelvedata),
 * a single object is wrapped in an array, and arrays are passed through.
 */
function serializeSymbols(items) {
    if (items.length === 1 && typeof items[0] === "string") {
        return items[0];
    }
    if (items.every((i) => typeof i === "string")) {
        return items.join(",");
    }
    return items.map((i) => (typeof i === "string" ? { symbol: i } : i));
}
/**
 * Inspect a non-101 upgrade response and return the most specific error class.
 * Twelvedata returns a JSON body like `{"code":401,"message":"...","status":"error"}`
 * for authentication failures, but the HTTP status itself can vary
 * (observed: 200 with an error body, 401, 403).
 */
function classifyUpgradeFailure(status, body) {
    var _a, _b;
    let parsed = null;
    try {
        parsed = JSON.parse(body);
    }
    catch (_c) {
        /* not JSON — fall through */
    }
    const message = (_a = parsed === null || parsed === void 0 ? void 0 : parsed.message) !== null && _a !== void 0 ? _a : (body ? body.slice(0, 200) : `HTTP ${status} from WebSocket upgrade`);
    const code = (_b = parsed === null || parsed === void 0 ? void 0 : parsed.code) !== null && _b !== void 0 ? _b : status;
    const isAuth = code === 401 ||
        code === 403 ||
        status === 401 ||
        status === 403 ||
        /api[\s_-]?key|unauthor|forbidden|invalid token/i.test(message);
    if (isAuth) {
        return new errors_1.WebSocketAuthError(`Authentication failed: ${message}`);
    }
    return new errors_1.WebSocketConnectionError(`Unexpected server response (HTTP ${status}): ${message}`);
}
function canonicalKey(item) {
    var _a, _b, _c;
    if (typeof item === "string")
        return `s:${item}`;
    return `o:${JSON.stringify({
        symbol: item.symbol,
        exchange: (_a = item.exchange) !== null && _a !== void 0 ? _a : "",
        mic_code: (_b = item.mic_code) !== null && _b !== void 0 ? _b : "",
        type: (_c = item.type) !== null && _c !== void 0 ? _c : "",
    })}`;
}
