"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.instanceOfGetTimeSeriesSarExt200ResponseMetaIndicator = instanceOfGetTimeSeriesSarExt200ResponseMetaIndicator;
exports.GetTimeSeriesSarExt200ResponseMetaIndicatorFromJSON = GetTimeSeriesSarExt200ResponseMetaIndicatorFromJSON;
exports.GetTimeSeriesSarExt200ResponseMetaIndicatorFromJSONTyped = GetTimeSeriesSarExt200ResponseMetaIndicatorFromJSONTyped;
exports.GetTimeSeriesSarExt200ResponseMetaIndicatorToJSON = GetTimeSeriesSarExt200ResponseMetaIndicatorToJSON;
exports.GetTimeSeriesSarExt200ResponseMetaIndicatorToJSONTyped = GetTimeSeriesSarExt200ResponseMetaIndicatorToJSONTyped;
/**
 * Check if a given object implements the GetTimeSeriesSarExt200ResponseMetaIndicator interface.
 */
function instanceOfGetTimeSeriesSarExt200ResponseMetaIndicator(value) {
    if (!("name" in value) || value["name"] === undefined)
        return false;
    if (!("startValue" in value) || value["startValue"] === undefined)
        return false;
    if (!("offsetOnReverse" in value) || value["offsetOnReverse"] === undefined)
        return false;
    if (!("accelerationLimitLong" in value) ||
        value["accelerationLimitLong"] === undefined)
        return false;
    if (!("accelerationLong" in value) || value["accelerationLong"] === undefined)
        return false;
    if (!("accelerationMaxLong" in value) ||
        value["accelerationMaxLong"] === undefined)
        return false;
    if (!("accelerationLimitShort" in value) ||
        value["accelerationLimitShort"] === undefined)
        return false;
    if (!("accelerationShort" in value) ||
        value["accelerationShort"] === undefined)
        return false;
    if (!("accelerationMaxShort" in value) ||
        value["accelerationMaxShort"] === undefined)
        return false;
    return true;
}
function GetTimeSeriesSarExt200ResponseMetaIndicatorFromJSON(json) {
    return GetTimeSeriesSarExt200ResponseMetaIndicatorFromJSONTyped(json, false);
}
function GetTimeSeriesSarExt200ResponseMetaIndicatorFromJSONTyped(json, ignoreDiscriminator) {
    if (json == null) {
        return json;
    }
    return {
        name: json["name"],
        startValue: json["start_value"],
        offsetOnReverse: json["offset_on_reverse"],
        accelerationLimitLong: json["acceleration_limit_long"],
        accelerationLong: json["acceleration_long"],
        accelerationMaxLong: json["acceleration_max_long"],
        accelerationLimitShort: json["acceleration_limit_short"],
        accelerationShort: json["acceleration_short"],
        accelerationMaxShort: json["acceleration_max_short"],
    };
}
function GetTimeSeriesSarExt200ResponseMetaIndicatorToJSON(json) {
    return GetTimeSeriesSarExt200ResponseMetaIndicatorToJSONTyped(json, false);
}
function GetTimeSeriesSarExt200ResponseMetaIndicatorToJSONTyped(value, ignoreDiscriminator = false) {
    if (value == null) {
        return value;
    }
    return {
        name: value["name"],
        start_value: value["startValue"],
        offset_on_reverse: value["offsetOnReverse"],
        acceleration_limit_long: value["accelerationLimitLong"],
        acceleration_long: value["accelerationLong"],
        acceleration_max_long: value["accelerationMaxLong"],
        acceleration_limit_short: value["accelerationLimitShort"],
        acceleration_short: value["accelerationShort"],
        acceleration_max_short: value["accelerationMaxShort"],
    };
}
